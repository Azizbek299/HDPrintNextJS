"use client"
import {getOurServicesByNameForUser } from '@/actions/actions';
import ServiceBoxImage2 from '@/components/Service/ServiceBoxImage2';
...
const page = ({params}:any) => {
  ...
  useEffect(()=> {
    async function getData(){
      setisLoading(true)
      let data = await getOurServicesByNameForUser(params.product)
      setOurServices(data.result)
      setisLoading(false)
    }
    getData()
  }, [])
  ...
  return (
    <div>
      <div className="">
        <ServiceBoxImage2 ourServices={ourServices}/>
      </div>
    </div>  
  )
}
